...
from price.utils.validator import is_web_page
...
from price.modules.price.parser.visit import Visit
from price.modules.price.parser.page import Page
from price.modules.price.parser.gallery import GaleryParser
...
class Services():
    ...
    def tags_ofert(self, ofert_id=None, shop_id=None, entry_point_id=None, scan_date=None):
        ps = ProductSupport()
        tp = TagsProduct()
        for lp, ofert in enumerate(ps.parase_all_ofert(ofert_id, shop_id, entry_point_id, scan_date)):
            if lp % 1000 == 0:
                log.info('Parse: \t\t\t %r oferts', lp)
            tp.tag_parser(ofert)

    def get_ofert_for_tags(self, ofert_id=None, shop_id=None, entry_point_id=None, scan_date=None):
        ps = ProductSupport()
        # tp = TagsProduct()
        for lp, ofert in enumerate(ps.parase_all_ofert(ofert_id, shop_id, entry_point_id, scan_date)):
            # tp.tag_parser(ofert)
