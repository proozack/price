...
from price.utils.string_utils import StringUtils
from price.modules.price.db_utils import TagProductDbUtils, TagOfertDbUtils, TagDbUtils, BrandDbUtils, CategoryDbUtils
from price.modules.price.tools import BrandTools, CategorySynonymTools
...
class TagsProduct():
    ...
    def tag_parser(self, ofert: Ofert = None, d_ofert: dict = None):
        su = StringUtils()
        if ofert:
            tp = TempProduct(**self.sa_obj_to_dict(ofert))
        else:
            tp = TempProduct(**d_ofert)

        tp = self.manage_category(tp)
        tp = self.manage_manufacturer(tp)        

        log.warning('Processing {}'.format(tp.title))
